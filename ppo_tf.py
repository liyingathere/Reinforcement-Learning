...
class PPO(object):
    def __init__(self):
        ...
        with tf.variable_scope("sample_action"):
            self.sample_op = tf.squeeze(oldpi.sample(1), axis=0)  # choosing action
            ...
            if METHOD["name"] == "kl_pen":
                ...
                self.aloss = -tf.reduce_mean(
                    tf.minimum(
                        surr,
                        tf.clip_by_value(
                            ratio, 1.0 - METHOD["epsilon"], 1.0 + METHOD["epsilon"]
                        )
                        * self.tfadv,
                    )
                )
        ...
        if METHOD["name"] == "kl_pen":
            for _ in range(A_UPDATE_STEPS):
                ...
                if kl > 4 * METHOD["kl_target"]:  # this in in google's paper
                    break
            if (
                kl < METHOD["kl_target"] / 1.5
            ):  # adaptive kl penalty, this is in OpenAI's paper
                METHOD["lam"] /= 2
            ...
            [
                self.sess.run(
                    self.atrain_op, {self.tfs: s, self.tfa: a, self.tfadv: adv}
                )
                for _ in range(A_UPDATE_STEPS)
            ]
    ...
    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            ...
            # Actor网络计算出一个均值和方差，并返回一个由该均值和方差得到的正态分布，动作基于此正态分布进行采样
